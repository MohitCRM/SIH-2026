  ...
  return (
    <div className="max-w-4xl mx-auto fade-in p-8">
      <header className="mb-10 flex justify-between items-center border-b border-base-200 pb-6">
        <div>
          <h1 className="text-3xl font-bold mb-1 text-primary">My Messages</h1>
          <p className="text-base-content/60 font-medium">Important notifications and updates about your applications</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="alert alert-error shadow-sm mb-6">
          <AlertCircle size={20} />
          <span>{error}</span>
          <button className="btn btn-sm btn-outline" onClick={fetchMessages}>Retry</button>
        </div>
      ) : messages.length === 0 ? (
        <div className="card bg-base-100 border-dashed border-2 border-base-300 p-10 text-center fade-in">
          ...
          <h3 className="text-lg font-bold text-base-content/80 mb-1">No Messages</h3>
          <p className="text-sm text-base-content/50 font-medium">You don't have any messages yet.</p>
        </div>
      ) : (
                ...
                {msg.actionUrl && (
                  <div className="card-actions justify-end mt-4 pt-4 border-t border-base-100">
                    <a href={msg.actionUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary btn-sm gap-2">
                      <ExternalLink size={16} /> Open Sanction Letter
                    </a>
                  </div>
                )}
              </div>
